...
public class EventTask extends BukkitRunnable {
    ...
    public EventTask(ZurixGens plugin, EventController eventController, int duration) {
        this.plugin = plugin;
    ...
    public void run() {
        if (duration <= 0) {
            EventType eventType = EventType.values()[ThreadLocalRandom.current().nextInt(0, EventType.values().length)];
